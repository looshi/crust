...
export function initializeState(URL) {
  // Master.
  Master = {
    id: 'master',
...
function initOscillator(URL, name, id, color) {
  return {
    ...
    note: 24, // The numeric keyboard note, e.g. A is 48.  Lowest C is zero.
    color,
    files: OSC_WAV_FILES,
    glide: URL.g || 0
  };
...
function EffectsReducer(state, action) {
  switch (action.type) {
    ...
    case 'ARP_IS_ON':
      ...
      updateURL('ao', state.arpIsOn ? 1 : 0);
      return Object.assign({}, state);
    default:
      return state;
  }
